...
Each item has a requested `service`, integer `quantity` (1–1000), optional `description` and optional integer `estimatedWeightGrams`. Up to 30 lines are accepted. Services are customer requests, not validated catalogue selections or quotes. Preferred drop-off is a request, not a guaranteed appointment. The first version allows dates from today to 90 days ahead in the branch timezone.
...
1. Customer form, service terms/version, owner enable/pause controls and private inbox UI; accessible errors and persistent retry handling.
